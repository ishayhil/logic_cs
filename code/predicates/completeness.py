...
def is_primitively_closed(sentences: AbstractSet[Formula]) -> bool:
    ...
    relations = set()
    for sentence in sentences:
        for t in sentence.relations():
            relations.add(t)

    for relation, arity in relations:
        for comb in product(consts, repeat=arity):
            if Formula(relation, [Term(c) for c in comb]) not in sentences and \
                    Formula('~', Formula(relation, [Term(c) for c in comb])) not in sentences:
                return False
    return True
...
def is_universally_closed(sentences: AbstractSet[Formula]) -> bool:
    ...
    for sentence in sentences:
        if sentence.root == 'A' and not all(
                sentence.predicate.substitute({sentence.variable: Term(c)}) in sentences for c in constants):
            return False
...
def is_existentially_closed(sentences: AbstractSet[Formula]) -> bool:
    ...
    constants = get_constants(sentences)
    for sentence in sentences:
        if sentence.root == 'E' and not any(
                sentence.predicate.substitute({sentence.variable: Term(c)}) in sentences for c in constants):
            return False
...
def find_unsatisfied_quantifier_free_sentence(sentences: Container[Formula],
    ...
    for c in model.constant_meanings:
        new_formula = unsatisfied.predicate.substitute({unsatisfied.variable: Term(c)})
        if new_formula in sentences and not model.evaluate_formula(new_formula):
            return find_unsatisfied_quantifier_free_sentence(sentences, model, new_formula)
    return unsatisfied
...
def model_or_inconsistency(sentences: AbstractSet[Formula]) -> \
    ...
    constants = {k: k for k in get_constants(sentences)}
    universe = get_constants(sentences)
    relations: Dict[str, Set[Tuple[str]]] = dict()
    for sentence in sentences:
        if is_relation(sentence.root):
            if sentence.root not in relations:
                relations[sentence.root] = set()
            relations[sentence.root].add(tuple(repr(c) for c in sentence.arguments))
    model = Model(universe, constants, relations)
    if all(model.evaluate_formula(sentence) for sentence in sentences):
        return model

    unsetisfied = None
    for sentence in sentences:
        if not model.evaluate_formula(sentence):
            unsetisfied = sentence
            break

    neg = set()
    not_neg = set()
    unsetisfied = find_unsatisfied_quantifier_free_sentence(sentences, model, unsetisfied)

    for r in get_primitives(unsetisfied):
        if tuple(repr(arg) for arg in r.arguments) in model.relation_meanings[r.root]:
            not_neg.add(r)
        else:
            neg.add(Formula('~', r))

    prover = Prover({*Prover.AXIOMS, *neg, *not_neg, unsetisfied})
    for asm in [*neg, *not_neg]:
        prover.add_assumption(asm)

    not_unsetisfied = Formula('~', unsetisfied)
    step1 = prover.add_tautological_implication(not_unsetisfied, {i for i in range(len([*neg, *not_neg]))})
    step2 = prover.add_assumption(unsetisfied)
    prover.add_tautological_implication(Formula("&", unsetisfied, not_unsetisfied), {step1, step2})
    proof = prover.qed()
    return proof
...
def combine_contradictions(proof_from_affirmation: Proof,
    ...
    # Task 12.4
    asmp = proof_from_affirmation.assumptions.intersection(proof_from_negation.assumptions)
    the_asm = list(proof_from_affirmation.assumptions.difference(asmp))[0].formula

    proof1 = proof_by_way_of_contradiction(proof_from_affirmation, the_asm)
    proof2 = proof_by_way_of_contradiction(proof_from_negation, Formula('~', the_asm))

    prover = Prover({*Prover.AXIOMS, *asmp})
    step1 = prover.add_proof(proof1.conclusion, proof1)
    step2 = prover.add_proof(proof2.conclusion, proof2)
    prover.add_tautological_implication(Formula("&", the_asm, Formula('~', the_asm)), {step1, step2})
    return prover.qed()
...
def eliminate_universal_instantiation_assumption(proof: Proof, constant: str,
    ...
    asmp = proof.assumptions.difference({instantiation}).union(Prover.AXIOMS)
    prover = Prover(asmp, True)
    ...
    prover.add_tautological_implication(Formula("&", instantiation, proof.conclusion), {step2, step0})
    return prover.qed()
...
def universal_closure_step(sentences: AbstractSet[Formula]) -> Set[Formula]:
    ...
    for sentence in sentences:
        if sentence.root == 'A':
            temp.update({sentence.predicate.substitute({sentence.variable: Term(c)}) for c in constants})
...
def replace_constant(proof: Proof, constant: str, variable: str = 'zz') -> \
    ...
    new_schemes = []
    for assumption in proof.assumptions:
        new_schemes.append(Schema(assumption.formula.substitute({constant: Term(variable)}),
                                  assumption.templates))
    prover = Prover(new_schemes, False)
    for line in proof.lines:
        ...
        if isinstance(line, Proof.MPLine):
            ...
            for k, v in line.instantiation_map.items():
                if isinstance(v, Formula) or isinstance(v, Term):
                    new_map[k] = v.substitute({constant: Term(variable)})
                elif v == constant or v == Term(constant):
                    new_map[k] = variable
                else:
                    new_map[k] = v
            new_schema = Schema(line.assumption.formula.substitute({constant: Term(variable)}),
                                line.assumption.templates)
            prover.add_instantiated_assumption(new_formula, new_schema, new_map)
    return prover.qed()
...
def eliminate_existential_witness_assumption(proof: Proof, constant: str,
    ...
    proof_not_witness = proof_by_way_of_contradiction(proof, witness)
    proof_not_witness_var = replace_constant(proof_not_witness, constant)
    prover = Prover(proof_not_witness.assumptions, True)
    step1 = prover.add_proof(proof_not_witness_var.conclusion, proof_not_witness_var)

    mapping = {'zz': Term(existential.variable)}
    new_formula = proof_not_witness_var.conclusion.substitute(mapping)
    step2 = prover.add_free_instantiation(new_formula, step1, mapping)
    step3_existential = prover.add_assumption(existential)

    # proof Ax[~R(x)] = ~Ex[R(x)]
    false = Formula('&', new_formula, Formula('~', new_formula)).substitute(
        {existential.variable: Term(next(fresh_constant_name_generator))})
    iml_formula = Formula('->', new_formula.first, false)
    step4 = prover.add_tautological_implication(iml_formula, {step2})
    step5_ug_teo_imp = prover.add_ug(Formula('A', existential.variable, iml_formula), step4)
    step6_true = prover.add_tautology(Formula('~', false))
    es_mapping = {'R': new_formula.first.substitute({existential.variable: Term('_')}),
                  'Q': false,
                  'x': existential.variable
                  }
    es_inst = Prover.ES.instantiate(es_mapping)
    step7_es = prover.add_instantiated_assumption(es_inst, Prover.ES, es_mapping)
    step8_not_exist = prover.add_tautological_implication(Formula('~', existential),
                                                          {step6_true, step5_ug_teo_imp, step7_es})
    step9_contradiction = prover.add_tautological_implication(Formula('&', existential, Formula('~', existential)),
                                                              {step8_not_exist, step3_existential})
    return prover.qed()
...
def existential_closure_step(sentences: AbstractSet[Formula]) -> Set[Formula]:
    ...
    for sentence in sentences:
        if sentence.root == 'E':
            ...
            if all(predicate.substitute({sentence.variable: Term(const)}) not in sentences for const in constants):
                temp.add(predicate.substitute({sentence.variable: Term(next(fresh_constant_name_generator))}))
    return temp
